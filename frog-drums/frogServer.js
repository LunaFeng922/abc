...
io.on('connection', (socket) => {
    ...
    socket.on("my-role", function(data){
        ...
        conductor = socket.id;
        socket.emit("frogs-already-online", frogs); // send all existing frogs to the conductor as soon as they connect
        }
    })
    ...
    socket.on("disconnect", function(){
       ...
       // console.log(frogs);
        ...
        // check if the disconnected socket is a frog or the conductor
        ...
        // if its a frog
        if (idx>-1){
            frogs.splice(idx, 1);//which & how many to delete
        }
        console.log(frogs);
            // delete frog
        // if it's a cnductpr
